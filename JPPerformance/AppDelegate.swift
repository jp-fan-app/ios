...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        ReviewService.sharedInstance.increaseAppStartCount()
        AnalyticsService.sharedInstance.start()
        AdMobService.sharedService.start()

        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
            ReviewService.sharedInstance.requestReviewIfNeeded()
        }

        SyncService.shared.synchronizeIfNeeded()
